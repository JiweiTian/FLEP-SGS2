...
rlSocket       sock("192.168.1.2",502,1);
rlSharedMemory shm("/srv/automation/shm/modbusPLC.shm",10);
rlMailbox      mbx("/srv/automation/mbx/modbusPLC.mbx");
rlThread       thread;
rlThread       watchdog;
short          lifeCounter     = 0;
...
void *reader(void *arg)
{
  ...
  while((buflen = mbx.read(buf,sizeof(buf))) > 0)
  {
    ...
    if(ret < 0)
    {
      ...
      shm.write(PLC_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
    }
...
int modbusCycle(int offset, int slave, int function, int start_adr, int num_register)
{
  ...
  else
  {
    ...
    shm.write(PLC_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  }
...
int main(int ac, char **av)
{
  ...
  shm.write(PLC_LIFE_COUNTER_BASE,&lifeCounter,2);
  shm.write(PLC_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  shm.write(PLC_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
  while(1)
  {
    ...
    ret = modbusCycle(offset,1,2,1,5);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,1,1,6,2);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,1,3,1024,1);
    if(ret>0) offset += ret; else continue;
  }
