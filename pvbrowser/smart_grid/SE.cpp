...
rlSocket       sock("192.168.1.3",502,1);
rlSharedMemory shm("/srv/automation/shm/modbusSE.shm",10);
rlMailbox      mbx("/srv/automation/mbx/modbusSE.mbx");
rlThread       thread;
rlThread       watchdog;
short          lifeCounter     = 0;
...
void *reader(void *arg)
{
  ...
  while((buflen = mbx.read(buf,sizeof(buf))) > 0)
  {
    ...
    if(ret < 0)
    {
      ...
      shm.write(SE_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
    }
...
int modbusCycle(int offset, int slave, int function, int start_adr, int num_register)
{
  ...
  else
  {
    ...
    shm.write(SE_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  }
...
int main(int ac, char **av)
{
  ...
  shm.write(SE_LIFE_COUNTER_BASE,&lifeCounter,2);
  shm.write(SE_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  shm.write(SE_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
  while(1)
  {
    ...
    ret = modbusCycle(offset,2,3,100,2);
