...
rlSocket       sock("192.168.1.4",502,1);
rlSharedMemory shm("/srv/automation/shm/modbusMATLAB.shm",174);
rlMailbox      mbx("/srv/automation/mbx/modbusMATLAB.mbx");
rlThread       thread;
rlThread       watchdog;
short          lifeCounter     = 0;
...
void *reader(void *arg)
{
  ...
  while((buflen = mbx.read(buf,sizeof(buf))) > 0)
  {
    ...
    if(ret < 0)
    {
      ...
      shm.write(MATLAB_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
    }
...
int modbusCycle(int offset, int slave, int function, int start_adr, int num_register)
{
  ...
  else
  {
    ...
    shm.write(MATLAB_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  }
...
int main(int ac, char **av)
{
  ...
  shm.write(MATLAB_LIFE_COUNTER_BASE,&lifeCounter,2);
  shm.write(MATLAB_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  shm.write(MATLAB_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
  while(1)
  {
    ...
    ret = modbusCycle(offset,3,3,0,84);
